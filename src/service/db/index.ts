...
class DbService implements DbServiceClass{
    ...
    async initializeDB(workspacePath: string): Promise<void>{
        try{
            ...
            isAppFirstLoaded && await DBClient.getInstance(this.userSpace).seed.run({ directory: this.seedFilePath })
        }catch(err) {
            console.log(err)
        }
    }
    async backupOldVersionWorkspace(workspacePath: string): Promise<void>{
        try {
            const migrateList = await DBClient.getInstance(this.userSpace).migrate.list({ directory: this.migrationFilePath })
