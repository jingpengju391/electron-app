...
class HttpClient {
  ...
  constructor(url: string, method: string, data: any, options?: any) {
    ...
    this.request.use((config: AxiosRequestConfig) => {
    ...
    this.response.use((response: AxiosResponse) => JSON.parse(response?.request?.response ?? response), (error: AxiosError) => Promise.reject(error));
  }
